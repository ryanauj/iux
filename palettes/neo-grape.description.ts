...
export const description: StyleDescription = {
  ...
  summary:
    'Neo Grape is the deep-violet tuning of the light neubrutalism engine. Structure matches the rest of the neo-* ' +
    ...
    'weight 900 uppercased over Space Grotesk UI. The distinguishing move is the deep-violet primary `#7c3aed` — ' +
    'the only neo-* primary dark enough to require white inverse content (`intent.primary.content = #ffffff`) — ' +
    'paired with a lime `#a3e635` counter-pop carried by the focus ring, on a barely-violet `#faf7ff` paper field.',
  origin:
    'The post-2020 light neobrutalism wave — Gumroad, Linktree, bento.me, bento-grid SaaS landing pages — tuned ' +
    'to the violet / electric-purple brand register that runs through Stripe-adjacent dev tools, indie game pages, ' +
    'and the post-Squarespace creator-economy aesthetic. Neo Grape is the deep-jewel tuning of the family: one ' +
    'rich purple primary, one lime counter-pop, ink everywhere else.',
  signatures: [
    {
      label: 'Deep-violet primary `#7c3aed` with WHITE inverse content (the one neo-* palette that inverts)',
      detail:
        '`intent.primary.bg = #7c3aed` / `content: #ffffff` — every other neo-* sibling keeps ink-black content on a ' +
        'lighter fill (bubblegum-pink, electric-lime, tangerine-orange). Grape is dark enough that ink content fails the ' +
        'contrast floor, so the engine routes `content` to white. This is the load-bearing differentiator inside the family.',
    },
    {
      label: 'Lime `#a3e635` focus ring as the complementary counter-pop',
      detail:
        '`effect.focusRing.color = #a3e635` and `intent.success.bg = #a3e635` — lime carries focus AND success. The violet/' +
        'lime complementary pair is the brand cue: a wine-purple primary lifted by a fresh-leaf accent that escalates ' +
        'cleanly over both the violet fill and the paper field.',
    },
    {
      label: 'Barely-violet `surface.base` (`#faf7ff`) with a deeper violet sunken (`#f1ebff`)',
      detail:
        '`surface.base` is `#faf7ff` — a 2% lavender paper tint, distinguishing this palette from Neo Bubblegum (pink-tinted), ' +
        'Neo Citrus (yellow-tinted), and Neo Tangerine (orange-tinted). The cool undertone is what locks the grape register ' +
    ...
    },
    {
      ...
      detail:
        ...
        '`8px 8px 0 #0a0a0a`. Every `radius.*` slot from `sm` through `full` is `0`. Identical engine chassis to every ' +
        'neo-* sibling — stamped rectangles, offset shadows, depth from offset rather than blur or curve.',
    },
    {
  ...
  tokenEvidence: [
    ...
    },
    {
      ...
      note: '`#7c3aed` deep violet — the dark primary that distinguishes Neo Grape from the lighter neo-* siblings.',
    },
    {
      path: 'color.intent.primary.content',
      note: '`#ffffff` white — the only inverted-content move in the neo-* family. Every other sibling keeps ink-black content on a bright fill.',
    },
    {
      path: 'color.surface.base',
      note: '`#faf7ff` barely-violet paper — distinguishes this palette from the pink / yellow / orange-tinted siblings.',
    },
    {
      path: 'effect.focusRing.color',
      note: '`#a3e635` lime — the complementary counter-pop. Same value carries `intent.success.bg`.',
    },
    {
      path: 'color.content.link',
      note: '`#6d28d9` deeper violet — the link color sits a step darker than the primary fill.',
    },
    {
  ...
  recallAliases: ['neo grape', 'neo-grape', 'grape neobrutalism', 'violet neobrutalism', 'purple brutalism', 'bento brutalism'],
}
